...
---
下面是一份完整的接口调用文档示例，涵盖上传、上传转 PDF、下载、预览等接口说明。你可以根据项目实际情况和格式要求进行调整和补充。

---

# 文件管理服务 API 调用文档

本文件描述了文件管理服务中涉及的文件上传、转换、下载及预览接口的调用方式、请求参数、示例请求及响应说明。
...
- **接口地址**: `http://10.11.0.20:8088/api/upload`
- **请求方式**: `POST`
- **描述**:  
  直接上传文件到服务器，不进行格式转换。上传成功后，会将文件记录存入数据库，并返回文件ID。
...
- **请求参数** (multipart/form-data):
  ...
  | DocumentTypeID        | Integer      | 是       | 文档类型ID，对应特定文档类型。                     |
  | matchID               | String       | 是       | 匹配ID，用于标识业务关联。                         |
  | PlanTrackingNumber    | String       | 否       | 计划跟踪号（选填）。                               |
...
- **响应示例** (JSON):
  ...
  ```json
  {
      "code": 200,
      "msg": "文件上传成功并已存储到数据库",
      "data": "MPR20250217151544001" // 文件ID
  }
  ```

### 1.2 上传并转换为 PDF

- **接口地址**: `http://10.11.0.20:8088/api/uploadToPdf`
- **请求方式**: `POST`
- **描述**:  
  上传文件后，先将原始文件转换为 PDF 格式，再上传到 FTP 服务器。转换完成后，将 PDF 文件记录保存至数据库，并返回文件ID。

- **请求参数** (multipart/form-data):

  | 参数名称              | 类型         | 是否必填 | 说明                                               |
    |-----------------------|--------------|----------|----------------------------------------------------|
  | file                  | MultipartFile| 是       | 上传的文件（原始数据流）。                         |
  | DocumentTypeID        | Integer      | 是       | 文档类型ID，对应特定文档类型。                     |
  | matchID               | String       | 是       | 匹配ID，用于标识业务关联。                         |
  | PlanTrackingNumber    | String       | 否       | 计划跟踪号（选填）。                               |
...
- **响应示例** (JSON):
  ...
  ```json
  {
      ...
      "data": "SPR20250217151612345" // 文件ID
  }
  ```

---

下面是一份完整的接口调用文档示例，采用 Markdown 格式描述各个接口、请求参数、示例请求以及响应说明。你可以根据项目需要进一步补充和调整。
