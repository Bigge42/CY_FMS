...
### 8. **删除文件接口（根据 fileId 软删除并移动到历史目录）**

- **路径**:  
  `http://10.11.0.20:8088/fms/ftp/deleteByFileId`
...
| 参数名称       | 类型       | 必填 | 说明                          | 示例             |
|------------|----------|----|-----------------------------|----------------|
| `fileId`   | `String` | 是  | 文件ID，用于定位要删除的文件记录         | `"MAN20250218155312030"` |
| `updatedBy` | `String` | 否  | 更新人，未提供时默认 `"system"`     | `"admin"`     |

- **请求示例**:
  ```bash
  curl -X DELETE "http://10.11.0.20:8088/fms/ftp/deleteByFileId?fileId=MAN20250218155312030&updatedBy=admin"
  ```
...
**说明**:
- 该接口根据 `fileId` 查询对应的文件记录，并将文件移动到原文件目录下的 `history` 子目录。
- 若 `history` 目录不存在，会自动创建。
- 数据库记录会同步更新 `fileURL` 为历史目录路径，并标记删除状态。
